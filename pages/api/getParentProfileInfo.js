...
export default async function handler(req, res) {
	...
	try {
		let sql = `SELECT name, email, pfp, role, datejoined, age, ip FROM parents WHERE host = (SELECT host FROM parents WHERE token = ?)`;
		let familyRows = await getData(db, sql, [token]);
		
		sql = `SELECT name, email, pfp, role, datejoined, age, ip FROM children WHERE host = (SELECT host FROM parents WHERE token = ?)`;
		familyRows = familyRows.concat(await getData(db, sql, [token]));

		let host_id = await getData(db, `SELECT host FROM parents WHERE token = ?`, [token]);
		if (host_id.length == 0) {
